...
DECLARE @idSocio AS INT = 0 --16888;
...
SELECT e.idpersona, e.IdSocio,1,'Socio','Edad', e.Edad, e.puntos FROM dbo.fnPLDmatrizEvaluacionEdades(@fechaTrabajo,@idSocio) e
...
INSERT @calificaciones(IdPersona,IdSocio,IdFactor,Factor,Elemento,Valor,Puntos)
			...
			FROM(
					SELECT p.IdPersona, sc.IdSocio, 'Socio' AS Factor,'TipoPersona' AS Elemento,
							...
							WHEN NOT pf.IdPersonaFisica IS NULL AND sc.ExentaIVA=1 THEN 2
...
FROM dbo.tSCSsocios sc  WITH(NOLOCK) 
INNER JOIN dbo.tSCSanalisisCrediticio ac  WITH(NOLOCK) ON ac.IdSocio = sc.IdSocio
...
SELECT * FROM @calificaciones cal ORDER BY cal.IdSocio
...
INNER JOIN dbo.tPLDmatrizConfiguracionNivelesRiesgo n ON c.Calificacion BETWEEN n.Valor1 AND n.Valor2