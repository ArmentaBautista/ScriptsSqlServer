...
IF EXISTS(SELECT name FROM sys.objects o WHERE o.name='pAYCdesagregadoOperacionesProductosFinancierosMetodoPago')
BEGIN
	DROP PROC pAYCdesagregadoOperacionesProductosFinancierosMetodoPago
	SELECT 'pAYCdesagregadoOperacionesProductosFinancierosMetodoPago BORRADO' AS info
END
GO

CREATE PROC pAYCdesagregadoOperacionesProductosFinancierosMetodoPago
...
BEGIN
		...
		INSERT INTO @ctas
		SELECT 
		c.IdCuenta, c.IdSocio, p.IdPersona, sc.Codigo, p.Nombre, c.Codigo, pf.Descripcion,p.RFC, tp.Descripcion,sc.IdListaDnivelRiesgo
...
INNER JOIN dbo.tCATlistasD ld  WITH(NOLOCK) 
	ON ld.IdListaD=c.IdListaDnivelRiesgo
LEFT JOIN @dg dg 
