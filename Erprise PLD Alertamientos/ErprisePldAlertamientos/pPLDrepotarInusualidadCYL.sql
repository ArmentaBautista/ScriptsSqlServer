...
CREATE OR ALTER  PROC  [dbo].[pPLDrepotarInusualidadCYL]
...
/* ?^•?•^?   JCA.04/10/2023.12:35 a. m. Nota: VARIABLES PARA USO O NO DE ALERTAS   */
...
DECLARE @fechaInicio AS DATE='19000101'
 ...
 SET @fechaFin= EOMONTH(@fecha,0)
	...
	---------------------------------------------------------------------------------------------------------------------
		DECLARE @NumeroOperacionesMesTodosMetodosPagos AS TABLE 
	(
		IdSocio int,
		IdTipoSubOperacion  int,
		MontoMes  numeric(23,8),
		NumeroOperacionesMes  INT,
		IdPersona INT
	)
		...
		SELECT 
		tabla.IdSocio,
		tabla.IdTipoSubOperacion,
		(SUM(tabla.Monto-tabla.MontoCambio)) AS MontoMes,
		COUNT(*) AS NumeroOperacionesMes ,
		IdPersona
		FROM (
				SELECT o.IdSocio,d.IdTipoSubOperacion,d.Monto,ISNULL(td.Monto,0) AS MontoCambio,o.IdPersona
				FROM tSDOtransaccionesD d(NOLOCK)
				JOIN tGRLoperaciones o (NOLOCK)
					ON d.IdOperacion=o.IdOperacion
				JOIN dbo.tSCSsocios soc (NOLOCK)
					ON soc.IdSocio=o.IdSocio 
				JOIN dbo.tSDOtransacciones t (NOLOCK)
					ON t.IdOperacion=o.IdOperacion 
						AND t.IdTransaccion=d.RelTransaccion
				left join tSDOtransaccionesD td (nolock)
					ON td.IdOperacion=o.IdOperacion 
						AND td.EsCambio=1
				WHERE  o.fecha BETWEEN   @fechaInicio AND   @Fecha 
					AND o.idsocio!=0
						AND t.IdTipoSubOperacion IN (500,501)  
							AND o.IdEstatus!=18  
								AND dbo.fnPLDesOperacionDotacionCustodia(o.IdOperacion)=0 
									AND NOT o.IdTipoOperacion IN (4,22,0,507,41) 
										AND d.EsCambio=0
				UNION all
				SELECT soc.idsocio,d.IdTipoSubOperacion, d.Monto,isnull(td.Monto,0) AS MontoCambio,soc.IdPersona
					...
					AND o.IdPersona=0  
						AND o.IdPersonaMovimiento!=0
							AND o.fecha BETWEEN   @fechaInicio AND @fecha 
								AND d.IdTipoSubOperacion IN (500,501)  
									AND o.IdEstatus!=18   
										AND  dbo.fnPLDesOperacionDotacionCustodia(o.IdOperacion)=0 
											AND NOT o.IdTipoOperacion IN (4,22,0,507,41) 
												AND d.EsCambio=0 
		 ) AS tabla --WHERE (IdPersona=@IdPersona OR @IdPersona=0)
		 GROUP BY IdSocio,IdTipoSubOperacion,IdPersona
	...
	DECLARE @TransaacionesDTodosMetodos AS TABLE
	(
		IdPersona INT,
	...
	SELECT 
	tabla.IdPersona, tabla.Monto, tabla.IdUsuarioAlta, tabla.Alta, tabla.IdUsuarioCambio,
	tabla.UltimoCambio, tabla.IdSesion, tabla.IdOperacion, tabla.IdTransaccionD, tabla.IdSocio,
	tabla.IdCuenta, tabla.IdEstatus, tabla.EsPersonaMoral, tabla.IdTipoSubOperacion, tabla.Fecha,
	tabla.IdDivisa, tabla.IdSucursal, tabla.IdMetodoPago
	FROM (
		SELECT 
		soc.IdPersona AS IdPersona,(d.Monto-ISNULL(td.Monto,0)) AS Monto,t.IdUsuarioAlta,t.Alta,t.IdUsuarioCambio,
		t.UltimoCambio,t.IdSesion,d.IdOperacion,d.IdTransaccionD,o.IdSocio,
		t.IdCuenta,o.IdEstatus,p.EsPersonaMoral,d.IdTipoSubOperacion,o.Fecha,
		t.IdDivisa,o.IdSucursal,d.IdMetodoPago
		FROM tSDOtransaccionesD d(NOLOCK)
		JOIN tGRLoperaciones o (NOLOCK)
			ON d.IdOperacion=o.IdOperacion
		JOIN dbo.tSDOtransacciones t (NOLOCK)
		...
		JOIN dbo.tSCSsocios soc(NOLOCK)
			ON soc.IdSocio=o.IdSocio
		JOIN dbo.tGRLpersonas p (NOLOCK)
			ON p.IdPersona=soc.IdPersona
		left join tSDOtransaccionesD td (nolock)
			ON td.IdOperacion=o.IdOperacion 
				AND td.EsCambio=1
		WHERE  o.fecha BETWEEN @fechaInicio AND @Fecha 
			AND o.idsocio!=0
				AND o.IdEstatus!=18 
					AND  dbo.fnPLDesOperacionDotacionCustodia(o.IdOperacion)=0 
						AND NOT o.IdTipoOperacion IN (4,22,0,507,41) 
							AND d.EsCambio=0 
								--AND (p.IdPersona=@IdPersona OR @IdPersona=0)
	  UNION ALL
		SELECT 
		soc.IdPersona,(d.Monto-ISNULL(td.Monto,0)) AS Monto,o.IdUsuarioAlta,o.Alta,
		o.IdUsuarioCambio,o.UltimoCambio,o.IdSesion,d.IdOperacion,d.IdTransaccionD,soc.IdSocio,
		0 AS IdCuenta,o.IdEstatus,per.EsPersonaMoral,d.IdTipoSubOperacion,o.Fecha,
		o.IdDivisa,o.IdSucursal,d.IdMetodoPago
		FROM tSDOtransaccionesD d(NOLOCK)
		JOIN tGRLoperaciones o (NOLOCK)
			ON d.IdOperacion=o.IdOperacion
		JOIN dbo.tSCSsocios soc (NOLOCK)
		...
		JOIN dbo.tGRLpersonas per With (nolock) 
			ON per.IdPersona = soc.IdPersona
		left join tSDOtransaccionesD td (nolock)
			ON td.IdOperacion=o.IdOperacion 
				AND td.EsCambio=1
		WHERE o.IdSocio=0 
			AND o.IdPersona=0  
				AND o.IdPersonaMovimiento!=0
					AND o.fecha BETWEEN @fechaInicio AND @Fecha AND o.idsocio!=0
						AND o.IdEstatus!=18 
							AND dbo.fnPLDesOperacionDotacionCustodia(o.IdOperacion)=0 
								AND NOT o.IdTipoOperacion IN (4,22,0,507,41) 
									AND d.EsCambio=0
	...
	BEGIN
		...
		,IIF(MontoSubOperacion=0,MontoReferencia,MontoSubOperacion)
		,1,-1,tabla.fecha,-1,GETDATE(),1598,IdSesion,IdOperacion
		,IdtransaccionD,CONCAT('Numero de depositos del mes:',(NumeroOperacionesMes ),',Limite superior declarado',RangoFin)
		,IdSocio
		,0
		,28
		,IdMetodoPago,'X',tabla.MontoMes
		FROM (
		...
		AND NOT EXISTS(SELECT IdTransaccionD FROM dbo.tPLDoperaciones 
		WHERE IdTransaccionD=tabla.IdtransaccionD AND IdInusualidad=29 AND Alta=tabla.Fecha)
  ...
  --                   MontoReferencia,IdSocio, IdCuenta,IdInusualidad,Descripcion)		
	...
	/*  (???)    JCA.04/10/2023.12:39 a. m. Nota: Se agrega validación para ejecutar o no dependiendo de como este configurado  */
	...
	SELECT 
	IdPersona,1593,46,MontoSubOperacion,1,-1,tabla.fecha,-1,GETDATE(),1598,IdSesion,IdOperacion
	,IdtransaccionD,CONCAT('Suma de depositos del mes:',MontoMes,',Limite superior declarado',RangoFin)
	,MontoMes,IdSocio,IdCuenta,24,IdMetodoPago,'X'
	FROM (
			SELECT 
			per.IdPersona,x.MontoMes,ope.Fecha
			,ope.IdSesion,ope.IdOperacion
			,t.IdTipoSubOperacion,t.IdtransaccionD
			,se.MontoDepositos AS RangoFin
			,soc.IdSocio,t.IdCuenta,t.IdMetodoPago
			,ROW_NUMBER()OVER(PARTITION BY t.IdSocio,t.IdPersona,t.IdTipoSubOperacion ORDER BY t.IdOperacion desc,ope.Fecha desc) AS numero
			,se.MontoDepositos 
			,t.MontoSubOperacion
			FROM @TransaacionesDTodosMetodos t
			JOIN @NumeroOperacionesMesTodosMetodosPagos x  
				ON x.IdSocio=t.IdSocio 
					AND x.IdTipoSubOperacion=500 
						--AND x.IdPersona=51338
			JOIN dbo.tGRLoperaciones ope With (nolock) 
				ON ope.IdOperacion = t.IdOperacion
			JOIN dbo.tSCSsocios soc With (nolock) 
				ON soc.IdSocio = t.IdSocio
			JOIN dbo.tGRLpersonas per With (nolock) 
				ON per.IdPersona = soc.IdPersona
			JOIN dbo.tSCSpersonasSocioeconomicos se With (nolock) 
				ON se.IdSocioeconomico = per.IdSocioeconomico																
	) AS tabla  
	WHERE tabla.IdTipoSubOperacion=500 
		AND tabla.MontoDepositos !=0 
			AND tabla.numero=1 
			AND (tabla.MontoMes>(tabla.RangoFin*3) or tabla.MontoMes>100000)
			AND NOT EXISTS(SELECT IdTransaccionD 
							FROM dbo.tPLDoperaciones  WITH(NOLOCK) 
							WHERE IdTransaccionD=tabla.IdtransaccionD 
								AND IdInusualidad=24 
	...
	FROM (
	  SELECT per.IdPersona,x.MontoMes,ope.Fecha,ope.IdSesion,ope.IdOperacion
	,t.IdtransaccionD,x.NumeroOperacionesMes,se.NumeroDepositos as RangoFin,t.IdTipoSubOperacion
	,soc.IdSocio,t.IdCuenta,t.IdMetodoPago,ROW_NUMBER()OVER(PARTITION BY t.IdSocio,t.IdPersona,t.IdTipoSubOperacion ORDER BY t.IdOperacion desc,ope.Fecha desc) AS numero
	,se.NumeroDepositos ,t.MontoSubOperacion
	FROM @TransaacionesDTodosMetodos t
	...
	WHERE tabla.IdTipoSubOperacion=500 AND (tabla.NumeroOperacionesMes) >tabla.RangoFin AND tabla.NumeroDepositos!=0 AND tabla.numero=1
	AND NOT EXISTS(SELECT IdTransaccionD FROM dbo.tPLDoperaciones WHERE IdTransaccionD=tabla.IdtransaccionD AND IdInusualidad=22 AND Alta=tabla.Fecha)
		...
		---------------------------------------------------------------------------------------------------------------------
	--------------------------Suma entrega de remesas en mes calendario = > 3,000 dls (Acumulado)----------------------
	----------------------------------------------------------------------------------
	-------------------------Mensual--------------------------------------------------------------------------------------
	---------------------------------------------------------------------------------------------------------------------
...
SELECT @fechaInicio=p.Inicio, @fechaInicio=p.Fin 
FROM dbo.tCTLperiodos p  WITH(NOLOCK) 
WHERE p.EsAjuste=0
	AND @fecha BETWEEN p.Inicio AND p.Fin
	
SELECT @FactorVentaOrigen = FactorVentaOrigen FROM dbo.tDIVfactoresTipoCambio  WITH(NOLOCK) WHERE IdDivisaOrigen=-4 AND fecha=@fecha
...
SELECT
 oper.IdPersonaMovimiento,1593,46,SUM(trans.SubTotalGenerado),1,-1,@fecha,-1,GETDATE(),1598,0,0,0,
...
WHERE trans.IdTipoSubOperacion = 516
	...
	AND trans.Fecha BETWEEN @fechaInicio AND @fechaFin
		AND NOT EXISTS(SELECT 
						IdTransaccionD 
						FROM dbo.tPLDoperaciones  WITH(NOLOCK) 
						WHERE IdSocio=socio.IdSocio 
							AND IdInusualidad=44 
								AND CAST(alta AS DATE)=@fecha)
GROUP BY 
...
HAVING ((@FactorVentaOrigen)* SUM(trans.SubTotalGenerado)) >= @MontoEnDolares
		...
		---------------------------------------------------------------------------------------------------------------------
	--------------------------Suma envio de remesas en mes calendario = > 3,000 dls (Acumulado)----------------------
	----------------------------------------------------------------------------------
	-------------------------Mensual--------------------------------------------------------------------------------------
	---------------------------------------------------------------------------------------------------------------------
...
SELECT
 oper.IdPersonaMovimiento,1593,46,SUM(trans.SubTotalGenerado),1,-1,@fecha,-1,GETDATE(),1598,0,0,0,
 'Suma envio de remesas en mes calendario = > 3,000 dls',SUM(trans.SubTotalGenerado),socio.IdSocio,0,47,'x'
FROM dbo.tSDOtransacciones trans WITH(NOLOCK)
INNER JOIN dbo.tGRLoperaciones oper WITH(NOLOCK) 
	ON oper.IdOperacion = trans.IdOperacion
		AND oper.IdOperacion != 0
...
LEFT JOIN dbo.tSCSsocios socio 
	ON socio.IdPersona = persona.IdPersona
WHERE trans.IdTipoSubOperacion = 513
	AND (au.IdAuxiliar IN (-4,-3,-5) OR remesa.EsOperacionesRemesas=1)
	AND trans.Fecha BETWEEN @fechaInicio AND @fechaFin
		AND NOT EXISTS(SELECT 
						IdTransaccionD 
...
HAVING ((@FactorVentaOrigen)* SUM(trans.SubTotalGenerado)) >= @MontoEnDolares
	...
	SELECT per.IdPersona,x.MontoMes,ope.Fecha,ope.IdSesion,ope.IdOperacion,t.IdTipoSubOperacion
	,t.IdtransaccionD,se.MontoRetiros as RangoFin
	,soc.IdSocio,t.IdCuenta,t.IdMetodoPago,ROW_NUMBER()OVER(PARTITION BY t.IdSocio,t.IdPersona,t.IdTipoSubOperacion ORDER BY t.IdOperacion desc,ope.Fecha desc) AS numero
	,se.MontoRetiros,t.MontoSubOperacion
	FROM @TransaacionesDTodosMetodos t
	...
	AND tabla.MontoRetiros!=0 AND tabla.numero=1
	...
	FROM (
	  SELECT per.IdPersona,x.MontoMes,ope.Fecha,ope.IdSesion,ope.IdOperacion
	,t.IdtransaccionD,x.NumeroOperacionesMes,se.NumeroRetiros as RangoFin,t.IdTipoSubOperacion
	,soc.IdSocio,t.IdCuenta,t.IdMetodoPago,ROW_NUMBER()OVER(PARTITION BY t.IdSocio,t.IdPersona,t.IdTipoSubOperacion ORDER BY t.IdOperacion desc,ope.Fecha desc) AS numero
	,se.NumeroRetiros,t.MontoSubOperacion
	FROM @TransaacionesDTodosMetodos t
	JOIN @NumeroOperacionesMesTodosMetodosPagos x  ON x.IdSocio=t.IdSocio AND x.IdTipoSubOperacion=501
	JOIN dbo.tGRLoperaciones ope With (nolock) ON ope.IdOperacion = t.IdOperacion
	JOIN dbo.tSCSsocios soc With (nolock) ON soc.IdSocio = t.IdSocio
	...
	WHERE tabla.IdTipoSubOperacion=501 AND (tabla.NumeroOperacionesMes) >tabla.RangoFin AND tabla.NumeroRetiros!=0 AND tabla.numero=1
	...
	WHERE   ope.Fecha=@Fecha AND t.IdMetodoPago =-2 AND c.IdCuenta!=0 AND t.Monto>50000
	AND ser.IdListaD IS NULL   AND ope.IdEstatus!=18 AND  dbo.fnPLDesOperacionDotacionCustodia(ope.IdOperacion)=0 
	AND NOT ope.IdTipoOperacion IN (4,22,0,507,41)-- AND se.IdRelServiciosFinancierosAsignados!=0
	)AS tabla
	WHERE tabla.numero=1
