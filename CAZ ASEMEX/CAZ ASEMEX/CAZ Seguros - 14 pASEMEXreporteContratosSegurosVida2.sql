...
IF @Tipo=2
BEGIN
	DECLARE @periodoAnterior VARCHAR(8)=(SELECT p.Codigo FROM dbo.tCTLperiodos p WHERE p.IdPeriodo=(@IdPeriodo-1))
...
, [Estatus]						= CASE 
								...
								END
FROM @contratos cs
LEFT JOIN dbo.vCTLDomiciliosPrincipales dom  WITH(NOLOCK) 
