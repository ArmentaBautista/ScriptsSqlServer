...
IF EXISTS(SELECT name FROM sys.objects o WHERE o.name='fnCnAYCcolocacion')
BEGIN
	DROP function dbo.fnCnAYCcolocacion
	SELECT 'fnCnAYCcolocacion BORRADO' AS info
END
GO

create function dbo.fnCnAYCcolocacion(
...
return
	...
	LEFT JOIN (SELECT  labSocio.Giro
						...
						--	LEFT JOIN dbo.tCTLlaborales labSocio WITH(NOLOCK) ON labSocio.IdPersona = persocio.IdPersona
						    LEFT JOIN (
										 SELECT laborales.IdPersona,laborales.Giro,
									 	 Numero = ROW_NUMBER()OVER(PARTITION BY laborales.IdPersona ORDER BY esLaboral.UltimoCambio DESC)
										 FROM tCTLlaborales laborales	
									     JOIN dbo.tCTLestatusActual esLaboral ON esLaboral.IdEstatusActual = laborales.IdEstatusActual AND esLaboral.IdEstatus = 1													
 ...
 where (@Sucursal='*'or sucsocio.Codigo = @Sucursal)
       and ce.FechaEntregada between @FechaInicial and @FechaFinal
