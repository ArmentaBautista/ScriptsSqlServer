...
IF EXISTS(SELECT name FROM sys.objects o WHERE o.name='pAYCsociosRestringidos')
BEGIN
	DROP PROC pAYCsociosRestringidos
	SELECT 'pAYCsociosRestringidos BORRADO' AS info
END
GO

CREATE PROC pAYCsociosRestringidos
AS
	BEGIN
	...
	UPDATE tAYCsociosRestringidos SET IdEstatus=2

	-- Socios con Saldo Igual o Mayor a X
	INSERT INTO tAYCsociosRestringidos (IdSocio,TipoBloqueo,IdEstatus)
	SELECT sc.IdSocio,1,1 -- Por Saldo
	...
	INSERT INTO tAYCsociosRestringidos (IdSocio,TipoBloqueo,IdEstatus)
	SELECT sc.IdSocio, 2,1 -- Por Nivel
	FROM dbo.tSCSsocios sc  WITH(NOLOCK) 
	...
	-- Se agrega por requerimiento para todos los socios con creditos superiores a 100000 de saldo 
		...
		INSERT INTO tAYCsociosRestringidos (IdSocio,TipoBloqueo,IdEstatus)
		SELECT c.IdSocio,1,1 -- Por Saldo
...
SELECT 'pAYCsociosRestringidos CREADO' AS info
GO
