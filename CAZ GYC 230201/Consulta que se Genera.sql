			...
			[Total Pagado]= (ISNULL(pagosCreditosActivos.CapitalPagadoNoAtrasado,0) + ISNULL(pagosCreditosCastigadosCondonados.CapitalPagado,0) ) +
							tf.InteresOrdinarioPagado + tf.IVAInteresOrdinarioPagado + tf.InteresMoratorioPagado + tf.IVAInteresMoratorioPagado + tf.CargosPagados  + tf.IVACargosPagado
FROM    dbo.fSDOtransaccionesFinancierasSaldosPagados('20221101','20221130') tf 
            INNER JOIN dbo.tAYCcuentas c WITH ( NOLOCK ) ON c.IdCuenta = tf.IdCuenta AND c.IdTipoDProducto = 143            
            ...
            LEFT JOIN ( SELECT  tf.Fecha ,tf.Idoperacion ,evento.IdCuenta ,evento.IdGestor,NumeroActividadesRango = COUNT(evento.IdActividad)
                        ...
                        WHERE   evento.IdEstatus = 50 AND evento.IdActividad != 0 AND evento.FechaRealizada BETWEEN DATEADD(DAY, -30,'20221101') AND '20221130' AND tf.Fecha BETWEEN '2022-11-01' AND '2022-11-30'		
			...
			LEFT JOIN dbo.fSDOpagosCreditosActivos('20221101','20221130') pagosCreditosActivos ON c.IdCuenta = pagosCreditosActivos.IdCuenta AND tf.Idoperacion = pagosCreditosActivos.IdOperacion
			...
			LEFT JOIN  dbo.tCNTauxiliares auxi WITH(NOLOCK) ON auxi.IdAuxiliar = trans.IdAuxiliar
WHERE   ((tf.Idoperacion != 0 AND tf.IdTipoOperacion IN(1, 10) AND tf.IdTipoSubOperacion = 500 AND tf.Idoperacion != 0 AND ((tf.Total > 0) or auxi.Codigo='TARJETA'))  
							OR (tf.Idoperacion != 0 AND tf.IdTipoOperacion IN(43) AND tf.IdTipoSubOperacion = 500 AND tf.Idoperacion != 0)) 
							AND ( g.Codigo = '*' OR '*' = '*' )  
							AND ( s.Codigo = '*' OR '*' = '*' )  
							AND ( tf.Fecha BETWEEN '20221101' AND '20221130' )
