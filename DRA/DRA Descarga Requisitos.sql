...
DECLARE @FechaInicial AS DATE='20200301'
DECLARE @FechaFinal AS DATE='20200330'
...
CASE WHEN regreq.IdTipoDdominio=208 THEN CONCAT(cuentaE.FolioConsultaSIC,'.',archivo.Extension) 
	 ELSE CONCAT('AVAL-',apertura.Folio,'-',cuentaE.FolioConsultaSIC,'.',archivo.Extension) 
	 END AS  NombreDescarga
...
INNER JOIN dbo.tAYCcuentas cuenta  WITH(NOLOCK) ON cuenta.IdCuenta=padre.IdRegistro
												AND cuenta.FechaAlta BETWEEN @FechaInicial AND @FechaFinal -- Filtro
