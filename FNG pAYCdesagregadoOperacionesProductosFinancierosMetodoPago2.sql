...
IF EXISTS(SELECT name FROM sys.objects o WHERE o.name='pAYCdesagregadoOperacionesProductosFinancierosMetodoPago2')
BEGIN
	DROP PROC pAYCdesagregadoOperacionesProductosFinancierosMetodoPago2
	SELECT 'pAYCdesagregadoOperacionesProductosFinancierosMetodoPago2 BORRADO' AS info
END
GO

CREATE PROC pAYCdesagregadoOperacionesProductosFinancierosMetodoPago2
...
INSERT INTO @tf
SELECT tf.IdTransaccion, tf.IdOperacion, tf.IdCuenta, tf.IdTipoSubOperacion,tt.Descripcion
, tf.MontoSubOperacion,tf.SaldoCapitalAnterior,tf.SaldoCapital
...
INNER JOIN dbo.fnGRLoperacionesConProductosFinancierosPorFecha(@FechaInicial,@FechaFinal) op  
...
Nota: Cargamos EstatusActua por tipo de dominio en memoria, queda pendiente revisar si es posible acotarlo por fecha
*/
DECLARE @ea TABLE(
	IdEstatusActual		INT,
