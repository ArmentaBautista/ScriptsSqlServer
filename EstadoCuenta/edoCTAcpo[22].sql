...
DECLARE @tEDOCTAsaldosaPagar AS TABLE(
	IdPeriodo					INT,	
	Periodo						VARCHAR(16),
	IdCuenta					INT,
	AbonoCapital				NUMERIC(10,2),
	InteresOrdinario			NUMERIC(10,2),
	InteresMoratorio			NUMERIC(10,2),
	SeguroVida					NUMERIC (10,2),
	CargosCobranza				NUMERIC (10, 2),
	IvaInteresOrdinario			NUMERIC(10,2),
	IvaInteresMoratorio			NUMERIC(10,2),
	IvaCargosCobranza			NUMERIC(10,2),
	IvaSeguroVida				NUMERIC(10,2),
	CapitalAtrasado				NUMERIC(10,2),
	DIasMora					NUMERIC(10,2)
)
...
INSERT @tEDOCTAdomicilios(IdRel,CP,Calle,Colonia,Localidad,Municipio,Estado,IdPeriodo,Periodo, NumeroExterior, NumeroInterior)
SELECT IdRel,dom.CodigoPostal,Calle,dom.Asentamiento,dom.Ciudad,Municipio,Estado,@idPeriodo,@periodo, dom.NumeroExterior, dom.NumeroInterior
...
FROM
    (
        VALUES
            ...
            ,('IVAIOCASTIGOCREDITO', 'IVA Castigado', 143, 22)
...
SELECT
    ...
    ,Abono = CASE ----------abonos-------------------
				...
				WHEN tipoMovimiento.IdTipoMovimiento = 'IMCASTIGADOCREDITO'
                 ...
                 /*iva castigado*/
                 WHEN tipoMovimiento.IdTipoMovimiento = 'IVAIOCASTIGOCREDITO'
                      AND tFinanciera.IdTipoSubOperacion = 14
                      AND ABS(ISNULL(tFinanciera.IVAcastigado, 0)) > 0 THEN
                     ABS(ISNULL(tFinanciera.IVAcastigado, 0))
                 ELSE
                     0
             END
    ...
    INNER JOIN @tEDOCTAtiposMovimiento tipoMovimiento
               ON (
					...
					*/
                      ...
                      OR
                          (
                              tipoMovimiento.IdTipoMovimiento = 'IVAIOCASTIGOCREDITO'
...
DELETE 
FROM IERP_OBL_REG.dbo.tEDOCTAcreditos
WHERE IERP_OBL_REG.dbo.tEDOCTAcreditos.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAcreditos (IdCuenta, IdSocio, IdProducto, NoCuenta, Producto, Finalidad, MontoOtorgado, FechaEntrega, TipoPlazo, NoPlazos, TipoPrestamo, FechaVencimiento,
							                  ProximoAbono, InteresOrdinario, InteresMoratorio, SaldoCapital, IdEstatus, IdPeriodo, Periodo
											  )		 
SELECT IdCuenta, IdSocio, IdProducto, NoCuenta, Producto, Finalidad, MontoOtorgado, FechaEntrega, TipoPlazo, NoPlazos, TipoPrestamo, FechaVencimiento, ProximoAbono, 
       InteresOrdinario, InteresMoratorio, SaldoCapital, IdEstatus, IdPeriodo, Periodo
FROM @tEDOCTAcreditos

DELETE 
FROM IERP_OBL_REG.dbo.tEDOCTAdatosSocio
WHERE IERP_OBL_REG.dbo.tEDOCTAdatosSocio.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAdatosSocio (IdSocio, IdPersona, NoSocio, Nombre, IdRelDomicilios, IdPeriodo, Periodo)
SELECT IdSocio, IdPersona, NoSocio, Nombre, IdRelDomicilios, IdPeriodo, Periodo
FROM @tEDOCTAdatosSocio

DELETE 
FROM IERP_OBL_REG.dbo.tEDOCTAdomicilios
WHERE IERP_OBL_REG.dbo.tEDOCTAdomicilios.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAdomicilios (IdRel, CP, Calle, Colonia, Localidad, Municipio, Estado, IdPeriodo, Periodo)
SELECT IdRel, CP, Calle, Colonia, Localidad, Municipio, Estado, IdPeriodo, Periodo
FROM @tEDOCTAdomicilios

DELETE 
FROM IERP_OBL_REG.dbo.tEDOCTAsaldos
WHERE IERP_OBL_REG.dbo.tEDOCTAsaldos.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAsaldos(IdPeriodo, Periodo, IdCuenta, CapitalPagado, InteresOrdinarioPagado, InteresMoratorioPagado, IVAInteresOrdinarioPagado,
...
DELETE 
FROM IERP_OBL_REG.dbo.tEDOCTAsegurosPagados
WHERE IERP_OBL_REG.dbo.tEDOCTAsegurosPagados.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAsegurosPagados(IdPeriodo, Periodo, IdCuenta, SegurosPagados, IVAsegurosPagados, CargosPagados, IVAcargosPagados)
SELECT IdPeriodo, Periodo, IdCuenta, SegurosPagados, IVAsegurosPagados, CargosPagados, IVAcargosPagados
FROM @tEDOCTAsegurosPagados

DELETE 
FROM IERP_OBL_REG.dbo.tEDOCTAmovimientos
WHERE IERP_OBL_REG.dbo.tEDOCTAmovimientos.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAmovimientos(IdCuenta, IdTransaccion, IdPeriodo, Fecha, Folio, Concepto, Cargo, Abono, Saldo, Orden)
SELECT IdCuenta, IdTransaccion, IdPeriodo, Fecha, Folio, Concepto, Cargo, Abono, Saldo, Orden
FROM @tEDOCTAmovimientos


DELETE
FROM IERP_OBL_REG.dbo.tEDOCTAsaldosaPagar
WHERE IERP_OBL_REG.dbo.tEDOCTAsaldosaPagar.IdPeriodo = @idPeriodo

INSERT INTO IERP_OBL_REG.dbo.tEDOCTAsaldosaPagar(IdPeriodo, Periodo, IdCuenta, AbonoCapital, InteresOrdinario, InteresMoratorio, SeguroVida, CargosCobranza,
												 IvaInteresOrdinario, IvaInteresMoratorio, IvaCargosCobranza, IvaSeguroVida, CapitalAtrasado, DIasMora
												)
SELECT IdPeriodo, Periodo, IdCuenta, AbonoCapital, InteresOrdinario, InteresMoratorio, SeguroVida, CargosCobranza,
												 IvaInteresOrdinario, IvaInteresMoratorio, IvaCargosCobranza, IvaSeguroVida, CapitalAtrasado, DIasMora
FROM @tEDOCTAsaldosaPagar
