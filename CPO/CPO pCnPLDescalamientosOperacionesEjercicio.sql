...
IF EXISTS(SELECT name FROM sys.objects o WHERE o.name='pCnPLDescalamientosOperacionesEjercicio')
BEGIN
	DROP PROC pCnPLDescalamientosOperacionesEjercicio
	SELECT 'pCnPLDescalamientosOperacionesEjercicio BORRADO' AS info
END
GO

CREATE PROC pCnPLDescalamientosOperacionesEjercicio
@periodo AS VARCHAR(8)=''
AS
BEGIN

	DECLARE @idPeriodo AS INT =0
	DECLARE @IdEjercicio AS INT=0
	SELECT @idPeriodo= p.idperiodo, @IdEjercicio= p.IdEjercicio FROM tCTLperiodos p  WITH(NOLOCK) WHERE p.Codigo=@periodo
	...
	INNER JOIN dbo.tCTLperiodos per  WITH(NOLOCK) ON per.IdPeriodo = acu.IdPeriodo
		AND per.IdEjercicio=@IdEjercicio
	WHERE acu.AcumuladoMesCalendario>=@MontoPersonasFisicas 
	--AND  acu.IdPeriodo=@idPeriodo 
	...
	INNER JOIN dbo.tCTLperiodos per  WITH(NOLOCK) ON per.IdPeriodo = acu.IdPeriodo
		AND per.IdEjercicio=@IdEjercicio
	WHERE  acu.AcumuladoMesCalendario>=@MontoPersonasMorales 
	--AND acu.IdPeriodo=@idPeriodo
	--ORDER BY acu.AcumuladoMesCalendario DESC
	...
	-- Consulta
	SELECT a.Periodo,
       suc.Descripcion AS SucursalSocio,
       a.NoSocio,
       p.Nombre,
       a.EsPersonaFisica,
       a.ExentaIVA AS TieneActividadEmpresarial,
       a.EsPersonaMoral,
       FORMAT(a.AcumuladoMesCalendario,'C','es-MX') AS AcumuladoMesCalendario,
       a.Alertamiento AS Escalamiento
	FROM @alertamientos a
		INNER JOIN dbo.tGRLpersonas p WITH (NOLOCK)
			ON p.IdPersona = a.IdPersona
		INNER JOIN dbo.tCTLsucursales suc WITH (NOLOCK)
			ON suc.IdSucursal = a.IdSucursal
